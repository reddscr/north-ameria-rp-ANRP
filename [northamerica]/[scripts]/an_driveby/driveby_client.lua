...
function toggleDriveby()
	...
	if ( theVehicle ) then
	if ( getElementModel ( theVehicle ) == 522 ) or ( getElementModel ( theVehicle ) == 581 ) or ( getElementModel ( theVehicle ) == 509 ) or ( getElementModel ( theVehicle ) == 481 ) or ( getElementModel ( theVehicle ) == 462 ) or ( getElementModel ( theVehicle ) == 521 ) or ( getElementModel ( theVehicle ) == 463 ) or ( getElementModel ( theVehicle ) == 510 ) or ( getElementModel ( theVehicle ) == 461 ) or ( getElementModel ( theVehicle ) == 448 ) or ( getElementModel ( theVehicle ) == 468 ) or ( getElementModel ( theVehicle ) == 586 ) or ( getElementModel ( theVehicle ) == 523 ) then
	targetSpeed = getElementSpeed( getPedOccupiedVehicle ( localPlayer ))
	...
	if equipedWeapon == 0 then
		...
		if prevw and nextw then
			...
			--helpText:text( "Aperte '"..prevw.."' Ou '"..nextw.."' Para Trocar De Arma" )
			fadeInHelp()
...
function unbindFire2()
		local theVehicle = getPedOccupiedVehicle ( localPlayer )
		if ( theVehicle ) then
			for k,v in pairs (blockmotosmph) do
			if not ( getElementModel ( theVehicle ) == v )  then
			targetSpeed = getElementSpeed( getPedOccupiedVehicle ( localPlayer ))
			if targetSpeed > 95 then
				setPedDoingGangDriveby ( localPlayer, false )
				setPedWeaponSlot( localPlayer, 0 )
